...
describe("Robinhood deployment parity", () => {
  ...
  it("keeps deployed game selectors and tuple order stable", () => {
    ...
    expect(toFunctionSelector("roundFunding(uint256)")).toBe("0x87a214cb");
    expect(toFunctionSelector("fundRoundReserves(uint256,uint256,uint256)")).toBe("0xad2fbd57");

    const roundFunding = getAbiItem({ abi: burntatoAbi, name: "roundFunding" });
    expect(roundFunding.type).toBe("function");
    if (roundFunding.type === "function") {
      expect(roundFunding.outputs.map((output) => output.name)).toEqual([
        "winnerReserve",
        "recoveryReserve",
        "winnerSponsoredEth",
  ...
  it("keeps every app write and integration selector stable", () => {
    ...
    expect(selector(operatorRewardsAbi, "totalRegisteredOperators")).toBe("0x7b789f0b");
    expect(selector(operatorRewardsAbi, "registerBatch")).toBe("0x69e4863f");
    expect(selector(operatorRewardsAbi, "sync")).toBe("0xb1357bf9");
    expect(selector(operatorRewardsAbi, "syncBatch")).toBe("0x36ad7cd9");
