...
export async function discoverOwnedOperatorIds(client: PublicClient, account: Address): Promise<bigint[]> {
  ...
  const candidates = [...new Set(logs.map((log) => log.args.tokenId).filter((id): id is bigint => typeof id === "bigint"))];
  const owners = await Promise.all(candidates.map(async (id) => {
    try {
