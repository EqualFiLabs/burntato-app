...
| Indexer runtime | Ponder backfilled from blocks `112330669` and `112339401`; `/ready`, `/status`, `/events`, and deployment identity responded | Local PGlite and public RPC only; the deployed system has no tracked user events yet. Hosted database/RPC remain operational work. |
...
| Browser | Desktop `1440×900` and mobile `390×844` passed without horizontal overflow; all six routes rendered; live Portal quote/fees, Operator inventory/quote and invalid-ID state, disabled spectator writes, modal Escape/focus restoration, and enabled wallet CTAs were checked | Privy rejected the temporary local origin before authentication. Rendered wallet configuration is proven; tenant custom-chain execution is not. |
...
- Fund the faucet for `tester count × 200,000 STATICS + buffer` and distribute enough testnet ETH for gas and the live Genesis native fee.
